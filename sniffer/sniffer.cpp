#define _WINSOCK_DEPRECATED_NO_WARNINGS //для работы 
...
void AnalyzeIpPacket(char* buffer, int size, std::ofstream& file) // функция анализа IP-пакета
{
    ...
    //извлекаем четыре младших бита из первого байта IP - заголовка, 
    //используя побитовую операцию "И" с маской 0x0F, которая устанавливает все остальные биты в ноль.
    int ip_header_length = (*ip_header & 0x0F) * 4; // длина IP-заголовка
    // извлекаем вторую часть первого байта, делаем сдвиг для перевода в 10 систему
...
int main(int argc, char* argv[])
{
    ...
    std::ofstream log_file(log_file_path); // создание лог-файла
    if (!log_file.is_open()) // проверка на успешное создание лог-файла
    {
    ...
    SOCKET sniffer_socket = INVALID_SOCKET; // создание сокета для перехвата пакетов
    ...
    while (true) // бесконечный цикл для приема пакетов
    {
        ...
        {
            AnalyzeIpPacket(recv_buf, recv_size, log_file); // анализ полученного пакета
        }
    }
    log_file.close(); // закрытие файла
